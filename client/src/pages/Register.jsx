  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      <form onSubmit={handleSubmit} className="bg-primary/10 border border-primary/20 rounded-xl p-8 w-full max-w-md space-y-4">
        <h1 className="text-2xl font-semibold text-center mb-6">{t("create_account")}</h1>
        <div>
        ...
        <p className="text-center text-sm text-gray-400">
          {t("have_account")} <Link to="/login" className="text-primary hover:underline">{t("signin_link")}</Link>
        </p>
      </form>
    </div>
  );
